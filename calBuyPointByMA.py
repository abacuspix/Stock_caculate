...
def chooseStockFromMA120(stockList):
    ...
    for stockname in stockList:
        ...
        MA_500 = calMA(df, 500)
        #print (stockname, df.iloc[-1]['close'], df.iloc[-2]['close'])
        if (df.iloc[-1]['close'] - df.iloc[-1]['MA_120']) > 0 \
                and (df.iloc[-2]['close'] - df.iloc[-2]['MA_120']) < 0\
                and (df.iloc[-1]['MA_13']-df.iloc[-1]['MA_120']) > 0\
                and (df.iloc[-2]['MA_13']-df.iloc[-2]['MA_120']) < 0:
            with open(logName, 'a') as f:
                f.write(''.join(stockname)+'\n')
